...
class OrdenTrabajo(models.Model):
    ...
    informe_estado = models.CharField(max_length=50, default='PENDIENTE')
    informe_actualizado_en = models.DateTimeField(auto_now=True, null=True, blank=True)
    informe_version = models.IntegerField(default=1)
...
def ruta_imagenes_estudio(instance, filename):
    paciente = instance.orden.cita.paciente
    nombre_limpio = f"{paciente.nombre}_{paciente.apellido}".replace(" ", "_")
    # Genera: media/imagenes_estudio/orden_4_Maria_Lopez/archivo.jpg
    return os.path.join('imagenes_estudio', f'orden_{instance.orden.id}_{nombre_limpio}', filename)


class ImagenEstudio(models.Model):
    orden = models.ForeignKey(OrdenTrabajo, on_delete=models.CASCADE, related_name='imagenes')
    archivo = models.FileField(upload_to=ruta_imagenes_estudio)
