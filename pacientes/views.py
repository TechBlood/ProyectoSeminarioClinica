...
# 🔥 importación del conversor DICOM
...
def es_tecnico_o_radiologo(user):
...
@login_required
@user_passes_test(es_tecnico_o_radiologo)
...
def adjuntar_imagenes(request, orden_id):
    ...
    if request.method == 'POST':
        archivos = request.FILES.getlist('imagenes_dicom')

        if not archivos:
            messages.error(request, 'No se seleccionó ningún archivo.')
            return redirect('adjuntar_imagenes', orden_id=orden_id)

        convertidos_exito = 0

        for archivo in archivos:
            if archivo.name.startswith('.') or archivo.name.endswith('.db'):
                continue

            jpg_file = dicom_a_jpg_memoria(archivo)

            if jpg_file:
                nueva_img = ImagenEstudio(
                    orden=orden,
                    subida_por=request.user,
                    subida_en=timezone.now()
                )
                nueva_img.archivo.save(jpg_file.name, jpg_file, save=True)
                convertidos_exito += 1

        if convertidos_exito > 0:
            # Notificar al radiólogo que el estudio ya tiene imágenes cargadas
            ...
            Bitacora.registrar(
                ...
                descripcion=f'Se procesaron y adjuntaron {convertidos_exito} imágenes a la orden #{orden.id}'
            )

            messages.success(request, f'Se procesaron {convertidos_exito} imágenes. El estudio ha sido enviado al Radiólogo.')
            return redirect('dashboard')
        else:
            messages.error(request, 'No se encontraron archivos DICOM válidos.')

    return render(request, 'pacientes/adjuntar_imagenes.html', {'orden': orden})
...
def citas_procesadas(request):
    """
    Maneja tanto los "Estudios listos para diagnóstico" (botón verde)
    como el "Historial de Citas Procesadas".
    """
    ver = request.GET.get('ver', 'pendientes')

    if ver == 'historial':
        # Citas con informe completado por el radiólogo
        ordenes = (
            OrdenTrabajo.objects.filter(cita__estado=Cita.ESTADO_PROCESADA)
            .select_related('cita', 'cita__paciente', 'cita__tipo_estudio')
            .distinct()
            .order_by('-informe_creado_en')
        )
    else:
        # Por defecto: Estudios que ya tienen imágenes del técnico pero faltan de informe
        ordenes = (
